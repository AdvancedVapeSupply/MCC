...
8. Flash ESP32 (optional): If the --flash flag is used, erases and flashes the
   ESP32 device with the created image.
...
LVGL_MICROPYTHON_DIR = MPY_DIR  # Add this line to define the missing variable
mct_path = MCT_DIR

# Remove the problematic source command (it won't work this way)
# Instead, print instructions for manual activation
print("\nIMPORTANT: Before running this script, please ensure you've activated the virtual environment:")
...
def find_esp32_port():
    ...
    if sys.platform.startswith('win'):
        ...
        # Changed from ttyACM to ttyUSB for standard ESP32
        ports = glob.glob('/dev/ttyUSB[0-9]*')
    elif sys.platform.startswith('darwin'):
        ports = glob.glob('/dev/tty.SLAB_USBtoUART*')  # Updated for ESP32
    else:
...
micropython_firmware_source = "../lvgl_micropython/build/lvgl_micropy_ESP32_GENERIC.bin"
micropython_firmware_dest = "lvgl_micropy_ESP32_GENERIC.bin"
...
FIRMWARE_FILENAME = "lvgl_micropy_ESP32_GENERIC.bin"
firmware_source = os.path.join(LVGL_MICROPYTHON_DIR, "build", FIRMWARE_FILENAME)
firmware_dest = FIRMWARE_FILENAME
...
# Define the total flash size (4MB)
TOTAL_FLASH_SIZE = 4 * 1024 * 1024  # 4MB
            ...
            "builds": [
                {
                    ...
                    "parts": [
                        {
                            "path": "lvgl_micropy_ESP32_GENERIC.bin",
                            "offset": 0
                        },
                        {
                            "path": "mct.bin",
...
try:
    ...
    if args.flash:
        print_step(8, "Flash ESP32")
        print("Attempting to flash the ESP32...")
        ...
        if esp32_port is None:
            print("Error: No ESP32 device found. Please check the connection.")
            sys.exit(1)

        print(f"Found ESP32 port: {esp32_port}")
